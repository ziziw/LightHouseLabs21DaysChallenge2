...
function howDangerous(){
  if(isRock(arguments[0])){
    return 100;
  }else if(isCurrent(arguments[0])){
    return 50;
  }else{
    return null;
  }
...
function percentageRocks(){
  return Math.floor(allRocks().length / totalCells()*10000)/100;
}

function percentageCurrents(){
  return Math.floor(allCurrents().length / totalCells()*10000)/100;
}

function percentageReport(){
  let arr = [];
  arr.push(percentageRocks(), percentageCurrents());
  return arr;
}
...
function safetyReport(){
  let safetyGrid = GRID.map(function(row) {
...
function diffLetter(str1, str2){
  let newNumb = parseInt(str2.charCodeAt(0)) - parseInt(str1.charCodeAt(0));
  if (newNumb < 0){
    newNumb = -newNumb;
  }
  return newNumb;
...
function diffNumb(str1, str2){
  let newNumb = parseInt(str2.charAt(1)) - parseInt(str1.charAt(1));
  if (newNumb < 0){
    newNumb = -newNumb;
  }
  return newNumb;
...
function calcDistance(){
  let newLetter = diffLetter(arguments[0], arguments[1]);
  let newNumb = diffNumb(arguments[0], arguments[1]);
  let answer = Math.sqrt(Math.pow(newLetter, 2) + Math.pow(newNumb, 2));
